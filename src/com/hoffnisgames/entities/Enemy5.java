...
public class Enemy5 extends Entity {
	...
	private int masky = 13;
	private int maskx = 6;
	private int mwidth = 3;
	private int mheigth = 3;
	private int flife = 0;
	private int mflife = 15;
	
	public Enemy5(int x, int y, int width, int heigth, BufferedImage sprite) {
		super(x, y, width, heigth, null);
		
		sprites = new BufferedImage[2];
		sprites[0] = Game.spritesheet.getSprite(6*16, 9*16, 16, 16);
		sprites[1] = Game.spritesheet.getSprite(7*16, 9*16, 16, 16);
		...
		for(int i = 0; i < Game.bullets.size(); i++) {
			Entity e = Game.bullets.get(i);	
				if(Entity.isCollinding(this, e)) {
					...
					Particle.pcolor = 2;
					World.genparticle(15,(int) x, (int)y);
					return;
				}
		...
		for(int i = 0; i < Game.bull.size(); i++) {
			Entity e = Game.bull.get(i);	
				if(Entity.isCollinding(this, e)) {
					...
					//Game.bull.remove(i);
					Particle.pcolor = 2;
					World.genparticle(25,(int) x, (int)y);
					return;
				}
	...
	public void render(Graphics g) {
		...
		g.drawImage(sprites[index], this.getX()-Camera.x, this.getY()-Camera.y, null);
			...
			g.drawImage(Entity.ENEMY5_FEED, this.getX()-Camera.x, this.getY()-Camera.y, null);
